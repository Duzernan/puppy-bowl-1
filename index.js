const BootClass = ('2304-ftb-et-web-ft')

const AllURL = (`https://fsa-puppy-bowl.herokuapp.com/api/${BootClass}`)

async function fetchData() {

    try {
        
        let response = await fetch(`${AllURL}/players`)
...
async function renderData(){
    ...
    for(let i = 0; i < playersList.length; i++){
        ...
        newElement.addEventListener('click', () => {
            ...
            if(teamContainer.childElementCount <= 5){

                teamContainer.appendChild(newElement)
                button.remove()
            }
